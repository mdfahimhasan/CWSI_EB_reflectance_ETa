...
# This script is a sample code of estimating CWSI and ETa using IRT sensor's data
...
def adj_target_T(T_sensor, target_emissivity, T_background=-15):
    ...
    # T_target has to to be calculated at unit Kelvin first, then converted to deg C
    T_target = (((T_sensor + 273.15) ** 4 - (1 - target_emissivity) * (T_background + 273.15) ** 4) / target_emissivity) ** 0.25
...
def run_CWSI_ETa_model(input_csv, output_csv, Nir_col='R_nir', Red_col='R_red',
    ...
    for idx, row in output_df.iterrows():
        # calculating required values for each row
